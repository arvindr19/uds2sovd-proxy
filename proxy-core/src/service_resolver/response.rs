...
/// Fallback JSON key for SOVD `ObjectDataItem` responses where the value is
/// stored under a generic `"data"` field (used when MDD parameter names do
/// not match any key in the SOVD response map).
const SOVD_DATA_FIELD_KEY: &str = "data";
        ...
        let effective_sizes: Vec<usize> = active_params
            ...
            .map(|p| {
                ...
                let value = response_data
                    ...
                    .or_else(|| response_data.get(SOVD_DATA_FIELD_KEY));
